...
| 项目记忆前置 | 真实项目工作先读取 `project-memory-playbook`，记忆稀疏时先校准习惯，改过记忆时结束前推送 |
...
如果本轮工作更新了项目记忆，结束前必须把 `$HOME/Codex/project-memory` 单独提交并推送到 GitHub。不要每条小记录都立刻推送；按任务或重要里程碑批量提交，长任务在切换项目、交接或可能丢失上下文前做 checkpoint。
...
| `project-memory-playbook` | 项目记忆规范 | 负责读取、校准和更新私有 `project-memory` 仓库，沉淀跨项目偏好、习惯、风格和决策 | 开始项目工作前先同步读取记忆；记忆稀疏时主动校准习惯；改过记忆时结束前单独提交并推送 |
