...
- 交付页面、设计说明、组件说明和页面文案时，不得写入 `Codex`、`OpenAI`、`Claude`、`Gemini`、`Copilot`、`Cursor` 等平台来源信息
- 不得出现“由 AI 生成”“Generated by ...”“Created by ...”“由某平台创建”之类痕迹
- 默认直接输出业务页面本身，不要额外附加展示型大背景、概念海报、卖点介绍区或说明横幅，除非用户明确要求
- App、小程序和移动端页面默认禁止使用“大标题 + 小标题 + 功能说明”式展板文案来解释页面用途
