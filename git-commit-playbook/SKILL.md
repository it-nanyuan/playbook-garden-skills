...
3. 在写提交说明前先判断提交备注语言：优先沿用用户已明确的语言偏好、仓库历史提交语言或团队既有规范；只有语言不明确时，才先询问用户使用中文还是英文。
...
- 提交备注语言默认跟随已确认偏好；如果没有明确偏好，先问一次“提交备注使用中文还是英文？”再继续。
...
2. 仓库历史提交中占主导的语言
3. 团队或仓库已有规范要求
4. 如果以上都不明确，先询问用户使用中文还是英文

执行要求：

- 只在语言不明确时提问，不要每次都重复问
- 用户确认一次后，后续默认沿用，直到用户主动更改
- 如果仓库历史语言高度统一，可以先建议沿用该语言，再由用户决定是否切换
